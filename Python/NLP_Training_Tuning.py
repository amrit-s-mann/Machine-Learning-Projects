''' This sample program is part three of four coding labs in the course. 
The four coding labs demonstrate NLP techniques for classifying data. 
In this module, the focus is on model training, evaluating, training and testing.
'''
...
from plotly.offline import init_notebook_mode, iplot # #Plotly Tools
...
filesentences = 'DSC 4 Module 3 Sentences.txt'
...
''' PART 7 - Fitment to train and test the model

Splitting the input data into training and testing data-sets is an important step before the actual training, testing begins
To recap, the column "stemsentence" contains the input 
...
X_train_org, X_test_org, y_train_org, y_test_org = train_test_split(dfdata[['stemsentence']], dfdata['nsentiment'], random_state = 0,train_size=0.95)

for i in range(0, len(X_train_org.stemsentence)):
    stemsentence = re.sub('[^a-zA-Z]', ' ', X_train_org.stemsentence.iloc[i])
    stemsentence = stemsentence.lower()
    stemsentence = stemsentence.split()
    stemsentence = ' '.join(stemsentence)
    X_train.append(stemsentence)
    
    
for i in range(0, len(X_test_org.stemsentence)):
    stemsentence = re.sub('[^a-zA-Z]', ' ', X_test_org.stemsentence.iloc[i])
    stemsentence = stemsentence.lower()
    stemsentence = stemsentence.split()
    stemsentence = ' '.join(stemsentence)
    X_test.append(stemsentence)

#Countvectorizer and Tfidftransformer are for word counting and for tfidf weights respectively. 
#Tfidfvectorizer combines the functionality of everything

#,ngram_range=(2, 2) ( for bigram)
...
# Create a pipeline of different models

pipeline = Pipeline([('vect', tf_idf),
                     ('chi2',  SelectKBest(chi2, k=1200)),
                     ('logreg', LogisticRegression(random_state=0))])

models = pipeline.fit(X_train, y_train_org)
ytestorg = np.array(y_test_org)

# confusion matrix and classification report(precision, recall, F1-score)
print(classification_report(ytestorg, models.predict(X_test)))
print(confusion_matrix(ytestorg, models.predict(X_test)))

#logistic regression classifier

scikit_log_reg = LogisticRegression(verbose=1, solver='liblinear',random_state=1, C=1, penalty='l1',max_iter=10000)
logregmodel=scikit_log_reg.fit(X_train_tf,y_train_org)

pthreshold = 0.60
y_preds_test_prob = np.where(logregmodel.predict_proba(X_test_tf)[:,1]>pthreshold, 1, 0)
y_preds_test = logregmodel.predict(X_test_tf)

print('With 50%')
print(classification_report(ytestorg, models.predict(X_test)))

print('With 60%')
print(classification_report(ytestorg,y_preds_test_prob ))

print("Confusion matrix: With 50%")
print(metrics.confusion_matrix(y_test_org, y_preds_test))

print("Confusion matrix: With 60%")
print(metrics.confusion_matrix(y_test_org, y_preds_test_prob))
tn, fp, fn, tp = metrics.confusion_matrix(y_test_org, y_preds_test_prob).ravel()
